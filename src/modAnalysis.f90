...
subroutine analysisNasa(innpr,id,outFolder)
    ...
    enddo
    !print *,uvarn,vvarn,tvarn,zvarn,qvarn
    ...
    do i=1,nvars
      ...
      enddo
      !write(*,fmt='(A,3(E15.5,1X))')trim(varName(i)),scaleFactor(i),addOfset(i),missingV(i)
    end do
...
!GEOS.20200429_00+20200429_0900.nc4
!IC2020042900.gra
    foutName=innpr(len_trim(innpr)-16:len_trim(innpr)-9)
    foutName=trim(foutName)//innpr(len_trim(innpr)-7:len_trim(innpr)-6)
    ...
    write(*,fmt='(I2.2,A)') id,' - Escrevendo '&
          //trim(outFolder)//trim(foutname)//'.dat/.ctl/.gra'
...
!GEOS.20200429_00+20200429_0000.nc4
!Pega a hora
    gradsData=innpr(len_trim(innpr)-7:len_trim(innpr)-6)//":00z"
!Pega o dia
    gradsData=trim(gradsData)//innpr(len_trim(innpr)-10:len_trim(innpr)-9)
!Pega Mes
    gradsData=trim(gradsData)//monthName(innpr(len_trim(innpr)-12:len_trim(innpr)-10))
!Pega o ano
    gradsData=trim(gradsData)//innpr(len_trim(innpr)-16:len_trim(innpr)-12)
...
subroutine adjustMissingValues(nVar,nxGrib,nyGrib,nprz_grib2,missingValue &
    ...
    real(kind=kind_rb)  , intent(in) :: missingValue
  ...
  subroutine sm_nasa(smFile,outFolder)
	  !# _ 
	  !#
	  !# @note
	  !# 
...
!Fazendo os niveis iguais aos que o BRAMS usava
	...
	foutName=smFile(len_trim(smFile)-29:len_trim(smFile)-21)
    fOutName="SM.GEOS"//trim(fOutName)//"0000"
...
!GEOS.20200429_00+20200429_0000.nc4
!Pega a hora
    gradsData=smFile(len_trim(smFile)-7:len_trim(smFile)-6)//":00z"
!Pega o dia
    gradsData=trim(gradsData)//smFile(len_trim(smFile)-10:len_trim(smFile)-9)
!Pega Mes
    gradsData=trim(gradsData)//monthName(smFile(len_trim(smFile)-12:len_trim(smFile)-10))
!Pega o ano
    gradsData=trim(gradsData)//smFile(len_trim(smFile)-16:len_trim(smFile)-12)
